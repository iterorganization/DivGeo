...
c
    ...
    !! the error message and and a full command example
     ...
     &   "Example (terminal): "
      ...
      if (streql(wall_string, " ")) wall = shot
      if (streql(wall_run_string, " ")) wall_run = run
      if (.not.(0.lt.shot.and.shot.le.214748)) then
        write(0,*) 'Invalid shot number for equilibrium IDS'
        call exit(0)
      end if
      if (.not.(0.lt.wall.and.wall.le.214748)) then
      ...
      if (.not.(0.le.wall_run.and.wall_run.le.9999)) then
