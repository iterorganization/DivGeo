...
c=====================================================
c
      write(*,*) 'Program fiesta2dg'
      call open_files(' ')

      call rdfiesta(1,iret,title,date,ipestg,nr,nz,
     ,           rdim,zdim,zmsmid,rcntc,redge,rma,zma,psimin,psilim,
     ,           btorc,fg,pg,ffg,ppg,pfm,rgr,zgr)
      if(iret.ne.0) then
        print *,'==== fiesta2dg: error in rdfiesta. iret =',iret
        stop
      end if
c
      print *,'psilim = ',psilim
      ...
      if(iret.ne.0) then
        print *,'==== fiesta2dg: error in wreqdg. iret = ',iret
      end if
c
      end
