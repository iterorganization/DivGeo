...
c
      ...
      integer lwrptdg,lelem,lnode,lbound,lreg,lcount,lke
...
c
      if(kwrite.gt.0) then
        write(nwrite,2200) 1,npr,npz,mpr,mpz
      endif
c
c
c                        +---------------------------+
...
c
 ...
 2600 format(t10,'Newton'
     +,          t40,'   csi =',1p,e12.4
     +,          t70,'   eta =',e12.4)
 3000 format(i1,5('   ',/),t18,1p,e16.4,5e16.4)
...
c     -------------------
      function artri(x1,x2,x3,y1,y2,y3)
      implicit none
      real (kind=R8) :: artri
      real (kind=R8), intent(in) :: x1, x2, x3, y1, y2, y3

      artri = abs(x1*(y2-y3)+x2*(y3-y1)+x3*(y1-y2))
